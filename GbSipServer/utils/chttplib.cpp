...
namespace cmedia
{
	...
	{
		return httplib::GetRequest(rtsp_url, media_url);
	}
}
namespace httplib {
 ...
 */
	...
	static std::vector<std::string> split_vector(const std::string & s)
	{
	...
	bool GetRequest(const char * url, const char * media_url)
	{
		nlohmann::json response;
		std::vector<std::string> split_v = split_vector(url);
		// rtsp://127.0.0.1:5034/live/strig
		if (split_v.size() < 3)
		{
			return false;
		}
		std::string app = split_v[split_v.size() -2];
		std::string stream = split_v[split_v.size() -1];
		httplib::Client cli(media_url);
...
void ClientImpl::set_hostname_addr_map(const std::map<std::string, std::string> addr_map) {
...
void Client::set_hostname_addr_map(const std::map<std::string, std::string> addr_map) {
