...
// 对齐 libwebrtc RTPSender::CopyHeaderAndExtensionsToRtxPacket：
//  - marker / timestamp / csrc 透传
//  - 扩展按原始包中存在的项拷贝，但跳过 MID / RSID（未经协商不应出现在 RTX 上）
//  - TransportSequenceNumber 必须重新分配（RTX 是全新发送事件，TWCC 要求唯一递增）
void CopyHeaderAndExtensionsToRtxPacket(
	const libmedia_transfer_protocol::RtpPacketToSend& src,
	libmedia_transfer_protocol::RtpPacketToSend* dst,
	uint16_t transport_sequence) {
	using libmedia_transfer_protocol::RTPExtensionType;
	using libmedia_transfer_protocol::TransportSequenceNumber;

	dst->SetMarker(src.Marker());
	dst->SetTimestamp(src.Timestamp());
	const std::vector<uint32_t> csrcs = src.Csrcs();
	dst->SetCsrcs(csrcs);

	for (int i = static_cast<int>(libmedia_transfer_protocol::kRtpExtensionNone) + 1;
	     i < static_cast<int>(libmedia_transfer_protocol::kRtpExtensionNumberOfExtensions);
	     ++i) {
		const auto ext = static_cast<RTPExtensionType>(i);
		if (ext == libmedia_transfer_protocol::kRtpExtensionMid ||
		    ext == libmedia_transfer_protocol::kRtpExtensionRtpStreamId ||
		    ext == libmedia_transfer_protocol::kRtpExtensionTransportSequenceNumber) {
			continue;
		}
		if (!src.HasExtension(ext)) {
			continue;
		}
		rtc::ArrayView<const uint8_t> source = src.FindExtension(ext);
		if (source.empty()) {
			continue;
		}
		rtc::ArrayView<uint8_t> destination = dst->AllocateExtension(ext, source.size());
		if (destination.empty() || source.size() != destination.size()) {
			continue;
		}
		std::memcpy(destination.begin(), source.begin(), source.size());
	}
	dst->SetExtension<TransportSequenceNumber>(transport_sequence);
}
...
namespace gb_media_server
{
	...
	*/
		...
		, session_(s)
	{
		...
		// 与 rtc_sdp BuildAnswerSdp 实际声明的 extmap 严格一致：仅 abs-send-time(2)、transport-cc(3)
		// 不要在此处注册未协商的扩展（MID/RSID/RRID），否则会被写入包头，对端无法识别
	...
	uint16_t RtcInterface::NextOutboundTransportSequenceNumber() {
		return outbound_transport_seq_.fetch_add(1, std::memory_order_relaxed);
	}
	...
	*/
	bool RtcInterface::SendSrtpRtp(uint8_t* data, size_t  size)
	{
		...
		std::lock_guard<std::mutex> lock(outbound_srtp_mutex_);
...
#endif // 
		// srtp_send_session_->EncryptBuffer 为会话成员数组，RTP / RTCP 共用同一会话：
		// 必须与 SendSrtpRtp 共用同一把锁，覆盖加密 + 网络发送全过程（data 指向 EncryptBuffer）。
		std::lock_guard<std::mutex> lock(outbound_srtp_mutex_);
	...
	*/
	...
	{
		std::lock_guard<std::mutex> lock(rtp_video_packets_mutex_);
		uint32_t  seq_number = rtp_packet->SequenceNumber();
		rtp_video_packets_[seq_number] = std::move(rtp_packet);
	...
	*/
	void RtcInterface::RequestNack(const libmedia_transfer_protocol::rtcp::Nack& nack)
	{
		...
		for (const auto& packetid : nack.packet_ids()) {
			std::shared_ptr<libmedia_transfer_protocol::RtpPacketToSend> original_packet;
			{
				std::lock_guard<std::mutex> cache_lock(rtp_video_packets_mutex_);
				auto iter = rtp_video_packets_.find(packetid);
				if (iter == rtp_video_packets_.end() || !iter->second) {
					GBMEDIASERVER_LOG(LS_WARNING) << "session:   NACK request for seq:" << packetid
						<< " but packet not found in cache";
					continue;
				}
				original_packet = iter->second;
			}

			const uint16_t original_seq = original_packet->SequenceNumber();

			GBMEDIASERVER_LOG(LS_INFO) << "NACK request for seq:" << packetid
				<< ", original_seq:" << original_seq
				<< ", rtx_seq:" << video_rtx_seq_
				<< ", original_pt:" << static_cast<int>(original_packet->PayloadType())
			...
			rtx_packet->SetSequenceNumber(static_cast<uint16_t>(video_rtx_seq_++));

			const size_t original_payload_size = original_packet->payload_size();
			...
			rtx_payload[0] = static_cast<uint8_t>((original_seq >> 8) & 0xFF);
			rtx_payload[1] = static_cast<uint8_t>(original_seq & 0xFF);
			...
			GBMEDIASERVER_LOG(LS_INFO) << "RTX packet created: rtx_seq:" << (video_rtx_seq_ - 1)
				<< ", rtx_pt:" << static_cast<int>(rtx_packet->PayloadType())
				<< ", rtx_ssrc:" << rtx_packet->Ssrc()
				<< ", rtx_payload_size:" << rtx_packet->payload_size()
				<< ", original_seq_in_payload:" << original_seq;
			uint8_t* ptr = (uint8_t * )(rtx_packet->data());
			SendSrtpRtp(ptr, rtx_packet->size());
		}
	}
}